-- report_financial_account_provice_result
...
from report.financial_account_ori as a 
...
-- 组合以上表 输出结果数据1
drop table if exists report.financial_account_ccuscode_result;
create table if not exists report.financial_account_ccuscode_result 
select 
    a.ccuscode 
    ,a.date_
    ,a.balance_opening 
...
drop table if exists report.financial_account_province_result;
create table if not exists report.financial_account_province_result 
select 
    b.province
    ,a.date_
    ,year(a.date_) as year_
...
from report.financial_account_ccuscode_result as a 
left join edw.map_customer as b 
on a.ccuscode = b.bi_cuscode 
