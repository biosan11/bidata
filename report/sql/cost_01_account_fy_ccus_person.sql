...
-- CREATE TABLE `cost_01_account_fy_ccus_person` (
...
select @r:= case when  @ccusname = a.ccusname and @year_ = a.year_ then @r+1 else 1 end as rownum
      ...
      ,a.e_dt
			,@year_:=a.year_ as year_
  from (select * from report.cost_01_account_fy_ccus order by ccusname,s_dt) a,(select @r:=0,@ccusname:='',@year_:='') b
;
...
truncate table report.cost_01_account_fy_ccus_person;
insert into report.cost_01_account_fy_ccus_person
select a.cpersonname 
      ,a.fashengrq
      ,a.ccuscode
      ,a.ccusname
