...
--功能：主试剂对应的成效分析
------------------------------------------------------------------------------------------
--程序名称：ccus_03_cinv_effect_20.sql
--目标模型：ccus_03_cinv_effect_20
--源    表：pdm.ccus_03_item_effect_20,edw.x_cinv_relation,pdm.invoice_order
-----------------------------------------------------------------------------------------
--加载周期：日增
------------------------------------------------------------------------------------------
 --作者：jiangsh
...
select a.finnal_ccuscode as ccuscode
      ...
      ,min(ddate) as ddate
      ,TIMESTAMPDIFF(month,case when min(ddate) <= '2020-01-01' then '2020-01-01' else min(ddate) end,'2020-01-01') + 1 as mon_diff
      ,sum(a.inum_person) / (1 + TIMESTAMPDIFF(month,case when min(ddate) <= '2020-01-01' then '2020-01-01' else min(ddate) end,'2020-01-01')) as inum_person_mon 
      ,a.cinvcode
      ,a.cinvname
...
-- 获取20年所有客户的主试剂合集
drop table if exists report.mid1_ccus_03_cinv_effect_20;
create temporary table report.mid1_ccus_03_cinv_effect_20 as
select ccuscode
      ,ccusname
      ,a.cinvcode
      ,a.cinvname
...
-- 按照客户项目聚合
drop table if exists report.mid2_ccus_03_cinv_effect_20;
create temporary table report.mid2_ccus_03_cinv_effect_20 as
select ccuscode
      ,sum(cost) as cost
      ,a.cinvcode_main
  from report.mid1_ccus_03_cinv_effect_20 a
 group by ccuscode,cinvcode_main
;

-- 各个客户主试剂在同一个项目之间的占比
drop table if exists report.mid3_ccus_03_cinv_effect_20;
create temporary table report.mid3_ccus_03_cinv_effect_20 as
select a.ccuscode
      ,a.ccusname
      ,a.cinvcode
      ,a.cinvname
      ...
      ,a.cost / b.cost as bl
  from report.mid1_ccus_03_cinv_effect_20 a
  left join report.mid2_ccus_03_cinv_effect_20 b
...
-- 获取20年所有的客户、项目、主试剂的合集
drop table if exists report.mid4_ccus_03_cinv_effect_20;
create temporary table report.mid4_ccus_03_cinv_effect_20 as
select a.province
      ,a.city
      ,'终端客户' as type
  ...
  left join report.mid3_ccus_03_cinv_effect_20 b
...
-- 这里更新主试剂的金额，含税不含税
update report.mid4_ccus_03_cinv_effect_20 a
 inner join report.mid1_ccus_03_cinv_effect_20 b
    on a.ccuscode = b.ccuscode
...
update report.mid4_ccus_03_cinv_effect_20 a
 inner join report.ccus_03_item_effect_20 b
    on a.ccuscode = b.ccuscode
...
-- 插入jc开头检测量的数据
insert into report.mid4_ccus_03_cinv_effect_20
select a.province
      ,a.city
      ,'终端客户'
      ,a.ccuscode
...
-- 计算所有的利润
drop table if exists report.mid5_ccus_03_cinv_effect_20;
create temporary table report.mid5_ccus_03_cinv_effect_20 as
select a.province
      ,a.city
      ,a.type
      ,a.ccuscode
      ...
      ,a.invoice_amount_main - a.iunitcost_main + invoice_amount_child - iunitcost_child - amount_20 - insure_md as profit
      ,(a.invoice_amount_main - a.iunitcost_main + invoice_amount_child - iunitcost_child - amount_20 - insure_md) / inum_person_inv as profit_price
      ,(a.invoice_amount_main - a.iunitcost_main + invoice_amount_child - iunitcost_child - amount_20 - insure_md) / (invoice_amount_main + invoice_amount_child) as profit_margin
      ,a.profit_margin_item
  from report.mid4_ccus_03_cinv_effect_20 a
;

-- 计算公司平均利润单价
drop table if exists report.mid6_ccus_03_cinv_effect_20;
create temporary table report.mid6_ccus_03_cinv_effect_20 as
select cinvcode
      ,avg(profit_price) as profit_price
  from report.mid5_ccus_03_cinv_effect_20
 group by cinvcode
;

-- 计算省份平均利润单价
drop table if exists report.mid7_ccus_03_cinv_effect_20;
create temporary table report.mid7_ccus_03_cinv_effect_20 as
...
-- 计算20年预计发货人份数
drop table if exists report.mid9_ccus_03_cinv_effect_20;
create temporary table report.mid9_ccus_03_cinv_effect_20 as
...
-- 插入数据
truncate table report.ccus_03_cinv_effect_20;
insert into report.ccus_03_cinv_effect_20
select a.province
      ,a.city
      ,a.type
      ,null
  ...
  from report.mid5_ccus_03_cinv_effect_20 a
  left join report.mid6_ccus_03_cinv_effect_20 b
    on a.cinvcode = b.cinvcode
  left join report.mid7_ccus_03_cinv_effect_20 c
    on a.cinvcode = c.cinvcode
   and a.province = c.province
  left join report.mid9_ccus_03_cinv_effect_20 d
    on a.cinvcode = d.bi_cinvcode
   and a.ccuscode = d.bi_cuscode
;

update report.ccus_03_cinv_effect_20 a
inner join (select * from pdm.invoice_price where state = '最后一次价格' and left(ccuscode,2) = 'DL' and end_dt >= '2020-01-01' group by finnal_ccuscode,cinvcode) b
...
update report.ccus_03_cinv_effect_20 set ccuscode = finnal_ccuscode,ccusname = finnal_ccusname where ccuscode is null;

update report.ccus_03_cinv_effect_20 b set b.type = (case when left(b.ccuscode,2) = 'DL' then '代理商' when left(b.ccuscode,2) = 'ZD' then '终端客户' when left(b.ccuscode,2) = 'GR' then '个人客户' else '其他' end );

-- 更新前置项目表的客户
update report.ccus_03_item_effect_20 a
inner join report.ccus_03_cinv_effect_20 b
    on a.finnal_ccuscode = b.finnal_ccuscode
   and a.cinvcode = b.cinvcode_main
   set a.ccuscode = b.ccuscode
      ,a.ccusname = b.ccusname
...
-- 更新最终客户对应的客户情况
-- drop table if exists report.mid8_ccus_03_cinv_effect_20;
-- create temporary table report.mid8_ccus_03_cinv_effect_20 as
-- select a.ccuscode
--       ,a.ccusname
--       ,a.finnal_ccuscode
--       ,a.finnal_ccusname
...
-- 
-- update report.ccus_03_cinv_effect_20 a
--  inner join report.mid8_ccus_03_cinv_effect_20 b
