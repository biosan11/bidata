...
drop table if exists report.account_fy_date;
create table report.account_fy_date as
...
select c.cpersonname
  ...
  from (SELECT A.id,A.d_num,cpersonname,
         ...
         FROM
         (
         SELECT id,d_num,cpersonname,s_dt,e_dt FROM report.account_fy_mid1 ORDER BY id ASC
