...
--源    表：pdm.invoice_order,report.map_population
-----------------------------------------------------------------------------------------
--加载周期：日增
------------------------------------------------------------------------------------------
--作者：jiangsh
...
-- 市级计算，市级人口增加
drop table if exists report.map_population_pre;
create temporary table report.map_population_pre as
select a.city_get
      ,left(a.ddate,7) as y_mon
      ,a.item_code
      ,b.fin_data,a.province_get as province
 from report.x_ccus_delivery_sj a
 left join report.map_population b
   on a.city_give = b.city
  and left(a.ddate,7) = left(b.ddate,7)
;

-- 区县人口增加
insert into report.map_population_pre
select a.city_get
      ,left(a.ddate,7) as y_mon
      ,a.item_code
...
drop table if exists report.map_population_shou;
create temporary table report.map_population_shou as
select a.city_get as city
      ,a.y_mon
      ,a.item_code
      ,sum(a.fin_data) as fin_data,province
  from report.map_population_pre a
 group by city_get,y_mon,item_code
;


-- 送样区县处理
drop table if exists report.map_population_pre1;
create temporary table report.map_population_pre1 as
select a.city_give
      ,left(a.ddate,7) as y_mon
      ,a.item_code
      ,b.fin_data,a.province_get as province
...
-- 插入区县送出去的情况
insert into report.map_population_pre1
select a.city_give
      ,left(a.ddate,7) as y_mon
      ,a.item_code
      ,- b.inum_person,a.province_get as province
...
drop table if exists report.map_population_song;
create temporary table report.map_population_song as
select a.city_give as city
      ,a.y_mon
      ,a.item_code,province
      ,sum(a.fin_data) as fin_data
  from report.map_population_pre1 a
...
-- 除去收样地市其余的地市情况
drop table if exists report.map_population_else;
create temporary table report.map_population_else as
select a.city
      ,left(ddate,7) as y_mon
      ,a.fin_data
  from report.map_population a
  left join report.map_population_shou b
    on a.city = b.city
...
select a.province
  ...
  left join report.map_population_else b
    on a.city = b.city
   and a.y_mon = b.y_mon
...
select a.province
      ...
      ,'送样'
  from report.invoice_order_pre a
  left join report.map_population_song b
    on a.city = b.city
   and a.y_mon = b.y_mon
   and a.item_code = b.item_code
 where b.city is not null
...
select a.province
      ...
      ,a.y_mon
      ,a.fin_data
  from report.map_population_shou a
  left join (select * from report.ccus_02_market_price group by city,cinvcode) b
    on a.city = b.city
   and a.item_code = b.item_code
 where b.city is not null
...
select a.province
      ...
      ,a.y_mon
      ,a.fin_data
  from report.map_population_song a
  left join (select * from report.ccus_02_market_price group by city,cinvcode) b
    on a.city = b.city
   and a.item_code = b.item_code
 where b.city is not null
