...
insert into report.account_fy_mid1 select * from report.account_fy_mid11 group by ccusname,e_dt order by ccusname,s_dt;


drop table if exists report.account_fy_ccus;
create table report.account_fy_ccus as
...
select c.d_num
  ...
  from (SELECT A.id,s_dt,ccuscode,
         ...
         FROM
         (
         SELECT id,d_num,s_dt,e_dt,ccusname,ccuscode FROM report.account_fy_mid1 ORDER BY id ASC
...
update report.account_fy_ccus set d_num = 0 where d_num > 1;
update report.account_fy_ccus set diff_dt = 1 where d_num = 0;
...
--          FROM
--          (
--          SELECT id,d_num,s_dt,e_dt,ccusname FROM report.account_fy_mid1 ORDER BY id ASC
...
-- update report.account_fy_ccus set e_dt = null where d_num <> 1;
