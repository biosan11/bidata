...
-- 抽取增量数据
create temporary table edw.customer_pre as
select a.*
      ,localtimestamp()
...
-- 防重跑机制
-- delete from edw.customer where end_date >= '${start_dt}' and end_date < '3000-12-31';

-- 更新历史数据
-- PDATE edw.customer set end_date = '${start_dt}' where ccuscode in (select ccuscode from edw.customer_pre);
...
select a.db
      ,a.ccuscode
      ,a.ccusname
      ,case when b.ccuscode is null then '请核查' else b.ccuscode end as bi_cuscode
      ,case when b.ccuscode is null then '请核查' else b.ccusname end as bi_cusname
      ,a.ccusabbname
      ,a.ccccode
      ,a.cdccode
      ,a.ccusdepart
      ...
      ,'3000-12-31'
      ,localtimestamp()
  from edw.customer_pre a
  left join (select * from edw.dic_customer  where left(db,10) = 'UFDATA_889' ) b
    on a.ccuscode = b.ccuscode
 where left(a.db,10) = 'UFDATA_889'
;
...
select a.db
      ...
      ,localtimestamp()
  from edw.customer_pre a
  left join (select * from edw.dic_customer  where left(db,10) <> 'UFDATA_889' group by ccuscode) b
    on a.ccuscode = b.ccuscode
 where left(a.db,10) <> 'UFDATA_889'
;

delete from edw.customer where ccusname = '测试555555';
